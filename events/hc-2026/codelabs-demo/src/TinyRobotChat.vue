...
import { modelProviders } from './tiny-robot-chat/config/chat-runtime'
import { useWindow } from './tiny-robot-chat/composables/useWindow'
import GenuiSwitch from './tiny-robot-chat/genui/GenuiSwitch.vue'
import { createChatProviderRuntime } from './tiny-robot-chat/genui/provider-runtime'
import { createGenuiSchemaStreamPlugin } from './tiny-robot-chat/genui/stream-plugin'
import { createBusinessSkillPlugin, loadBusinessSkills } from './tiny-robot-chat/skills/business-skills'
import { createWebMcpAdapter } from './tiny-robot-chat/webmcp/webmcp-adapter'

const genuiUrl = import.meta.env.VITE_GENUI_URL?.trim()
const genuiPromptId = import.meta.env.VITE_GENUI_PROMPT_ID?.trim()
const genuiApiKey = import.meta.env.VITE_GENUI_API_KEY?.trim()
const genuiAvailable = Boolean(genuiUrl && genuiPromptId)
const genuiEnabled = ref(false)
const isGenuiActive = (): boolean => genuiEnabled.value && genuiAvailable

const providerRuntime = createChatProviderRuntime({
  modelProviders,
  isGenuiEnabled: isGenuiActive,
  genuiUrl,
  genuiPromptId,
  genuiApiKey,
})
...
onUnmounted(() => webMcpAdapter.dispose())

const businessSkills = shallowRef<SkillDefinition[]>([])
const businessSkillPlugin = createBusinessSkillPlugin(businessSkills)
void loadBusinessSkills()
  .then((skills) => {
    businessSkills.value = skills
  })
  .catch((error: unknown) => {
    console.warn('[tiny-robot-chat] failed to load business skills', error)
  })
...
const runtime = useLocalChatRuntime({
  conversation: {
    useMessageOptions: {
      ...
      plugins: [createGenuiSchemaStreamPlugin(isGenuiActive), businessSkillPlugin],
    },
  },
  mcp: webMcpAdapter,
})
...
const chatUi = computed(() =>
  createChatUi({ floatingOptions: window.floatingOptions, templateExtensions, isGenerating }),
)
  ...
  <TrTheme>
    ...
    >
      ...
      <template #sender-footer>
        <GenuiSwitch :enabled="genuiEnabled" :available="genuiAvailable" @toggle="handleGenuiToggle" />
      </template>
    </TrChat>
  </TrTheme>
</template>
