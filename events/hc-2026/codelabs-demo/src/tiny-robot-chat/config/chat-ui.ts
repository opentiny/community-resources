import type { BubbleMessage, ChatMessageContent, ChatMessageContentItem, LayoutFloatingOptions } from '@opentiny/tiny-robot'
...
import { h, type ComputedRef, type CSSProperties } from 'vue'
import { createGenuiRendererMatch } from '../genui/renderer.ts'
...
function resolveBubbleContent(message: BubbleMessage): ChatMessageContent | undefined {
  const genuiContent = (message.state as { genuiContent?: unknown } | undefined)?.genuiContent
  return Array.isArray(genuiContent) && genuiContent.length > 0
    ? (genuiContent as ChatMessageContentItem[])
    : message.content
}
  ...
  return {
    ...
    bubble: {
      ...
      bubbleList: {
        ...
        contentResolver: resolveBubbleContent,
        roleConfigs: {
      ...
      bubbleProvider: {
        contentRendererMatches: [createGenuiRendererMatch(options.isGenerating)],
      },
    },
    layout: {
      heightMode: 'parent',
